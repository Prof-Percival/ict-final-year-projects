...
trnpse: 
	lbu $t6, 9($t2)   # Load B[i] 
	sb  $t6, 0($t1)   # Store B[i] into it's transpose (BT[i])
	addi $t2, $t2, 3  # Offset for the Transpose
	addi $t0, $t0, 3  # Increase counter in 3
...
printMatrixAElements: 
	...
	addi $t9, $t9, 1
	beq $t9, 3, matrixANewLine # If all the numbers of the Matrix A haven't been printed, do all the same process
...
printMatrixBElements: 
	...
	addi $t9, $t9, 1
	beq $t9, 3, matrixBNewLine # If all the numbers of the Matrix B haven't been printed, do all the same process
...
loopi:  
    ...
    addi $s2, $s2, 3 # Increase in 3 the counter for loop i
